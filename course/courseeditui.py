...
class CourseEditUi(tk.Toplevel):
    def __init__(self):
        super().__init__()
        ...
        ttk.Label(
            self, text=' Detalhe da Disciplina', style='Title.TLabel'
        ...
        ttk.Label(row_2, text='Abbr', width=8).grid(row=0, column=0)
        self.entry_abbr = ttk.Entry(row_2, width=8)
        self.entry_abbr.grid(row=0, column=1, padx=3)
        row_2.grid(row=2, column=0)
        
        row_3 = ttk.Frame(profile)
        ttk.Label(row_3, text='Situação', width=8).grid(row=0, column=0)
        self.var_status = tk.IntVar(value=1)
        check_status = ttk.Checkbutton(row_3, text="Ativo",
